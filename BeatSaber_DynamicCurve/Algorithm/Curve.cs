...
namespace BeatSaber_DynamicCurve.Algorithm
{
    internal class Curve
    {
        ...
        public static double sliderMultiplier = 0.075;
        ...
        public static void Curves(List<ColorNoteData> notes, List<BombNoteData> bombs, float bpm)
        {
            ...
            for (int i = 0; i < curveX.Count(); i++)
            {
                if(i <= threshold)
                {
                    finalY[i] = curveY[i] * (1 + (sliderMultiplier * ((double)cube.Where(c => c.Slider).Count() / cube.Where(c => c.Head || !c.Pattern).Count() * 2)));
                }
                else
                {
                    finalY[i] = curveY[i] * (1 + (-sliderMultiplier * ((double)cube.Where(c => c.Slider).Count() / cube.Where(c => c.Head || !c.Pattern).Count() * 2)));
            ...
            Plugin.Log.Info(Math.Round((double)cube.Where(c => c.Linear).Count() / cube.Where(c => c.Head || !c.Pattern).Count() * 100, 2).ToString() + "% linear " + Math.Round((double)cube.Where(c => c.Slider).Count() / cube.Where(c => c.Head || !c.Pattern).Count() * 100, 2).ToString() + "% slider " + Math.Round(distance, 2) + " movement");

            for (int i = 0; i < curveX.Count(); i++)
            {
                finalY[i] *= 1 + (multiplier * (distance * 2));
